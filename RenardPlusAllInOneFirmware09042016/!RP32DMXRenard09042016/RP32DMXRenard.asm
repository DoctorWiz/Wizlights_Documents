...
;; filename: RP32 All in one firmware.asm
...
;;  and modified to accommodate the Renard Plus Controllers by Mac Macmillan (Phoenix)
;;
...
;;  PIN 7  (RA5) - triac driver 6 (output)
;;  PIN 8  (RE0) - triac driver 7 (output)
;;  PIN 9  (RE1) - triac driver 8 (output)
;;  PIN 10 (RE2) - triac driver 9 (output)
;;  PIN 11 (VDD) 
;;  PIN 12 (VSS) 
;;  PIN 13 (RA7) - triac driver 10 (output)
;;  PIN 14 (RA6) - triac driver 11 (output)
;;  PIN 15 (RC0) - triac driver 12 (output)
;;  PIN 16 (RC1) - triac driver 13 (output)
;;  PIN 17 (RC2) - triac driver 14 (output)
;;  PIN 18 (RC3) - triac driver 15 (output)
;;  Pin 19 (RD0) - triac driver 16 (output)
;;  Pin 20 (RD1) - Status LED ZC and HeartBeat, tri pulse beat
;;  PIN 21 (RD2) - triac driver 29 (output)
;;  PIN 22 (RD3) - triac driver 30 (output)
;;  PIN 23 (RC4) - triac driver 31 (output)
;;  PIN 24 (RC5) - triac driver 32 (output)
;;  PIN 25 (RC6/TX) - uart_out (output)
;;  PIN 26 (RC7/RX) - uart_in (input)
;;  PIN 27 (RD4) - triac driver 25 (output)
;;  PIN 28 (RD5) - triac driver 26 (output)
;;  PIN 29 (RD6) - triac driver 27 (output)
;;  PIN 30 (RD7) - triac driver 28 (output)
;;  PIN 31 (VSS)
;;  PIN 32 (VDD)
;;  PIN 33 (RB0) - triac driver 21 (output)
;;  PIN 34 (RB1) - triac driver 22 (output)
;;  PIN 35 (RB2) - triac driver 23 (output)
;;  PIN 36 (RB3) - triac driver 24 (output)
;;  PIN 37 (RB4) - triac driver 17 (output)
;;  PIN 38 (RB5) - triac driver 18 (output)
;;  PIN 39 (RB6) - triac driver 19 (output)
;;  PIN 40 (RB7) - triac driver 20 (output)
...
;; #DEFINE CLOCKRATE 32000000
 #DEFINE CLOCKRATE 64000000  ;;PIC18F45K22 ONLY
 
#ELSE

 #DEFINE CLOCKRATE 32000000  ;;PIC18F45xx
...
#define DMX_BLINK_RATE (44)


;;  Status LED Port D bit 3 only
...
;;**************************************************************************
;;************ AC INCANDESCENT/LED OR DC LED LIGHT DEFINITIONS *************
;;**************************************************************************

#DEFINE TRIAC_OR_SSR_LIGHTS  ;;used for Triac/SSR driven Icandescent or AC LED lights, AC Power
;;#DEFINE DC_LED_LIGHTS  ;;mostly used for ULN2803AN Darlington LED Drivers, DC power


#IFDEF TRIAC_OR_SSR_LIGHTS  ;;used for Triac/SSR driven Icandescent or LED lights, AC Power
...
;;  PortA, bit 7 has channel 10 mapped to it
  #DEFINE PIN13_CH 10
  #DEFINE PIN13_SINK YES
  #DEFINE PIN13_PWM YES

;;  PortA, bit 6 has channel 11 mapped to it
  #DEFINE PIN14_CH 11
  #DEFINE PIN14_SINK YES
  #DEFINE PIN14_PWM YES
;; ---------------------------- PORT B mapping -------------------------------

;;  PortB, bit 0 has channel 21 mapped to it
  #DEFINE PIN33_CH 21
  #DEFINE PIN33_SINK YES
  #DEFINE PIN33_PWM YES

;;  PortB, bit 1 has channel 22 mapped to it
  #DEFINE PIN34_CH 22
  #DEFINE PIN34_SINK YES
  #DEFINE PIN34_PWM YES

;;  PortB, bit 2 has channel 23 mapped to it
  #DEFINE PIN35_CH 23
  #DEFINE PIN35_SINK YES
  #DEFINE PIN35_PWM YES

;;  PortB, bit 3 has channel 24 mapped to it
  #DEFINE PIN36_CH 24
  #DEFINE PIN36_SINK YES
  #DEFINE PIN36_PWM YES

;;  PortB, bit 4 has channel 17 mapped to it
  #DEFINE PIN37_CH 17
  #DEFINE PIN37_SINK YES
  #DEFINE PIN37_PWM YES

;;  PortB, bit 5 has channel 18 mapped to it
  #DEFINE PIN38_CH 18
  #DEFINE PIN38_SINK YES
  #DEFINE PIN38_PWM YES

;;  PortB, bit 6 has channel 19 mapped to it
  #DEFINE PIN39_CH 19
  #DEFINE PIN39_SINK YES
  #DEFINE PIN39_PWM YES

;;  PortB, bit 7 has channel 20 mapped to it
  #DEFINE PIN40_CH 20
...
;;  PortC, bit 0 has channel 12 mapped to it
  #DEFINE PIN15_CH 12
  #DEFINE PIN15_SINK YES
  #DEFINE PIN15_PWM YES

;;  PortC, bit 1 has channel 13 mapped to it
  #DEFINE PIN16_CH 13
  #DEFINE PIN16_SINK YES
  #DEFINE PIN16_PWM YES

;;  PortC, bit 2 has channel 14 mapped to it
  #DEFINE PIN17_CH 14
  #DEFINE PIN17_SINK YES
  #DEFINE PIN17_PWM YES

;;  PortC, bit 3 has channel 15 mapped to it
  #DEFINE PIN18_CH 15
  #DEFINE PIN18_SINK YES
  #DEFINE PIN18_PWM YES

;;  PortC, bit 4 has channel 31 mapped to it
  #DEFINE PIN23_CH 31
  #DEFINE PIN23_SINK YES
  #DEFINE PIN23_PWM YES

;;  PortC, bit 5 has channel 32 mapped to it
  #DEFINE PIN24_CH 32
  #DEFINE PIN24_SINK YES
  #DEFINE PIN24_PWM YES

;;  PortC, bits 6 and 7 are used for serial (UART) I/O
  #DEFINE PIN25_SINK NO  ;;TX UART
  #DEFINE PIN26_SINK NO  ;;RX UART

;; ---------------------------- PORT D mapping -------------------------------

;;  PortD, bit 0 has channel 16 mapped to it
  #DEFINE PIN19_CH 16
  #DEFINE PIN19_SINK YES
  #DEFINE PIN19_PWM YES

;;  PortD, bit 3 has channel 30 mapped to it
  #DEFINE PIN22_CH 30
  #DEFINE PIN22_SINK YES
  #DEFINE PIN22_PWM YES

;;  PortD, bit 2 has channel 29 mapped to it
  #DEFINE PIN21_CH 29
...
;;  PortD, bit 1 has LED driver mapped to it
  ...
  #DEFINE LED2_TRIS TRISD,1
  #DEFINE LED2_anode_sel portd_image,1

;;  PortD, bit 4 has channel 25 mapped to it
  #DEFINE PIN27_CH 25
  #DEFINE PIN27_SINK YES
  #DEFINE PIN27_PWM YES

;;  PortD, bit 5 has channel 26 mapped to it
  #DEFINE PIN28_CH 26
  #DEFINE PIN28_SINK YES
  #DEFINE PIN28_PWM YES

;;  PortD, bit 6 has channel 27 mapped to it
  #DEFINE PIN29_CH 27
  #DEFINE PIN29_SINK YES
  #DEFINE PIN29_PWM YES

;;  PortD, bit 7 has channel 28 mapped to it
  #DEFINE PIN30_CH 28
...
#ENDIF
;;************************************************************************
;;************************ DC LED LIGHT DEFINITIONS **********************
;;************************************************************************
#IFDEF DC_LED_LIGHTS  ;;mostly used for ULN2803AN Darlington LED Drivers, DC power
...
;;  PortB, bit 0 has channel 21 mapped to it
  #DEFINE PIN33_CH 21
  #DEFINE PIN33_SINK NO
  #DEFINE PIN33_PWM YES

;;  PortB, bit 1 has channel 22 mapped to it
  #DEFINE PIN34_CH 22
  #DEFINE PIN34_SINK NO
  #DEFINE PIN34_PWM YES

;;  PortB, bit 2 has channel 23 mapped to it
  #DEFINE PIN35_CH 23
  #DEFINE PIN35_SINK NO
  #DEFINE PIN35_PWM YES

;;  PortB, bit 3 has channel 24 mapped to it
  #DEFINE PIN36_CH 24
  #DEFINE PIN36_SINK NO
  #DEFINE PIN36_PWM YES

;;  PortB, bit 4 has channel 17 mapped to it
  #DEFINE PIN37_CH 17
  #DEFINE PIN37_SINK NO
  #DEFINE PIN37_PWM YES

;;  PortB, bit 5 has channel 18 mapped to it
  #DEFINE PIN38_CH 18
  #DEFINE PIN38_SINK NO
  #DEFINE PIN38_PWM YES

;;  PortB, bit 6 has channel 13 mapped to it
  #DEFINE PIN39_CH 19
  #DEFINE PIN39_SINK NO
  #DEFINE PIN39_PWM YES

;;  PortB, bit 7 has channel 20 mapped to it
  #DEFINE PIN40_CH 20
  #DEFINE PIN40_SINK NO
  #DEFINE PIN40_PWM YES
...
;;  PortC, bit 4 has channel 31 mapped to it
  #DEFINE PIN23_CH 31
  #DEFINE PIN23_SINK NO
  #DEFINE PIN23_PWM YES

;;  PortC, bit 5 has channel 32 mapped to it
  #DEFINE PIN24_CH 32
  #DEFINE PIN24_SINK NO
  #DEFINE PIN24_PWM YES
...
;;  PortD, bit 4 has channel 25 mapped to it
  #DEFINE PIN27_CH 25
  #DEFINE PIN27_SINK NO
  #DEFINE PIN27_PWM YES

;;  PortD, bit 5 has channel 26 mapped to it
  #DEFINE PIN28_CH 26
  #DEFINE PIN28_SINK NO
  #DEFINE PIN28_PWM YES

;;  PortD, bit 6 has channel 27 mapped to it
  #DEFINE PIN29_CH 27
  #DEFINE PIN29_SINK NO
  #DEFINE PIN29_PWM YES

;;  PortD, bit 7 has channel 28 mapped to it
  #DEFINE PIN30_CH 28
  #DEFINE PIN30_SINK NO
  #DEFINE PIN30_PWM YES
...
;; The following definitions probably won't need to be changed by the user.
  ...
  #ENDIF
  #IFDEF PIN23_CH
    #DEFINE PIN23_DIMMER (1<<4)  
  #ELSE
    #DEFINE PIN23_DIMMER 0
  #ENDIF
  #IFDEF PIN24_CH
    #DEFINE PIN24_DIMMER (1<<5)  
  #ELSE
...
;;   symbol 'OSC_CONTROL_LOCAL' is defined in ren16_local.inc.
 ...
 #define OSC_CONTROL_LOCAL 0x60  ;;64MHZ 
...
;; registers 0x6 (6) through 0x39 (57)
  ...
  ctr_ckt25, ctr_ckt26, ctr_ckt27, ctr_ckt28
  ctr_ckt29, ctr_ckt30, ctr_ckt31, ctr_ckt32
...
;; registers 0x40 (64) through 0x5F (95)
  ...
  slot_id_ckt25, slot_id_ckt26, slot_id_ckt27, slot_id_ckt28
  slot_id_ckt29, slot_id_ckt30, slot_id_ckt31, slot_id_ckt32
...
;----------------------------------------------------------------------
	...
	; Define PAYLOAD size
		movlw	0x20 					; 32 bytes of payload
...
rx_not_sync:
	...
	goto	mainloop
	btfsc	FSR0L,5		; skip if non-echo state
