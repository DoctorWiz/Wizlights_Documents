...
;;  PIN 10 (RA6) - triac driver 8 (output)
;;  PIN 9  (RA7) - triac driver 7 (output)
;;  PIN 11 (RC0) - LED driver (anode for RxD, cathode for FrameErr)
;;  PIN 12 (RC1) - LED driver (anode for HB, cathode for ZC)
;;  PIN 13 (RC2) - LED driver (anode for ZC, RxD, cathode for HB)
;;  PIN 14 (RC3) - not used
;;  PIN 15 (RC4) - not used
;;  PIN 16 (RC5) - not used
;;  PIN 17 (RC6/TX) - uart_out (output)
;;  PIN 18 (RC7/RX) - uart_in (input)
;;  PIN 21 (RB0) - triac driver 16 (output)
;;  PIN 22 (RB1) - triac driver 15 (output)
;;  PIN 23 (RB2) - triac driver 14 (output)
;;  PIN 24 (RB3) - triac driver 13 (output)
;;  PIN 25 (RB4) - triac driver 12 (output)
;;  PIN 26 (RB5) - triac driver 11 (output)
;;  PIN 27 (RB6) - triac driver 10 (output)
;;  PIN 28 (RB7) - triac driver 9 (output)
...
;; #DEFINE BAUDRATE 460800   ;;PIC18F25K22 Only
...
;;********************* Do Not Change *************************
 ...
 #DEFINE CLOCKRATE 32000000  ;;PIC18F2525
;; #DEFINE CLOCKRATE 64000000 ;;PIC18F25K22 
...
;;  PortA, bit 6 has channel 8 mapped to it
  #DEFINE PIN10_CH 8
  #DEFINE PIN10_SINK YES
  #DEFINE PIN10_PWM YES

;;  PortA, bit 7 has channel 7 mapped to it
  #DEFINE PIN9_CH 7
...
;;  PortB, bit 0 has channel 16 mapped to it
  #DEFINE PIN21_CH 16
  #DEFINE PIN21_SINK YES
  #DEFINE PIN21_PWM YES

;;  PortB, bit 1 has channel 15 mapped to it
  #DEFINE PIN22_CH 15
  #DEFINE PIN22_SINK YES
  #DEFINE PIN22_PWM YES

;;  PortB, bit 2 has channel 14 mapped to it
  #DEFINE PIN23_CH 14
  #DEFINE PIN23_SINK YES
  #DEFINE PIN23_PWM YES

;;  PortB, bit 3 has channel 13 mapped to it
  #DEFINE PIN24_CH 13
  #DEFINE PIN24_SINK YES
  #DEFINE PIN24_PWM YES

;;  PortB, bit 4 has channel 12 mapped to it
  #DEFINE PIN25_CH 12
  #DEFINE PIN25_SINK YES
  #DEFINE PIN25_PWM YES

;;  PortB, bit 5 has channel 11 mapped to it
  #DEFINE PIN26_CH 11
  #DEFINE PIN26_SINK YES
  #DEFINE PIN26_PWM YES

;;  PortB, bit 6 has channel 10 mapped to it
  #DEFINE PIN27_CH 10
  #DEFINE PIN27_SINK YES
  #DEFINE PIN27_PWM YES

;;  PortB, bit 7 has channel 9 mapped to it
  #DEFINE PIN28_CH 9
...
;;  PortC, bit 0 has LED driver mapped to it
  #DEFINE PIN11_SINK NO
  #DEFINE PIN11_PWM YES
  #DEFINE LED0_TRIS TRISC^BANK1,0
  #DEFINE LED0_anode_sel portc_image,0

;;  PortC, bit 1 has LED driver mapped to it
  #DEFINE PIN12_SINK NO
  #DEFINE PIN12_PWM YES
  #DEFINE LED1_TRIS TRISC^BANK1,1
  #DEFINE LED1_anode_sel portc_image,1

;;  PortC, bit 2 has LED driver mapped to it
  #DEFINE PIN13_SINK NO
  #DEFINE PIN13_PWM YES
  #DEFINE LED2_TRIS TRISC^BANK1,2
  #DEFINE LED2_anode_sel portc_image,2

;;  PortC, bit 3 not used
  #DEFINE PIN14_SINK NO
  #DEFINE PIN14_PWM YES
			
;;  PortC, bit 4 not used
  #DEFINE PIN15_SINK NO
  #DEFINE PIN15_PWM YES

;;  PortC, bit 5 not used
...
;----------------------------------------------------------------------
	...
	; Define PAYLOAD size
		movlw	0x10 					; 32 bytes of payload
...
rx_not_sync:
	...
	goto	mainloop
	btfsc	FSR0L,4		; skip if non-echo state,,RP32 6 RP24 5 RP16 4 TR8 3
